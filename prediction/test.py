...
def get_final_df(model, data):
    ...
    if SCALE:
        y_test = np.squeeze(data["column_scaler"][ADJCLOSE_COLUMN].inverse_transform(np.expand_dims(y_test, axis=0)))
        y_pred = np.squeeze(data["column_scaler"][ADJCLOSE_COLUMN].inverse_transform(y_pred))
...
if SCALE:
    mean_absolute_error = data["column_scaler"][ADJCLOSE_COLUMN].inverse_transform([[mae]])[0][0]
else:
